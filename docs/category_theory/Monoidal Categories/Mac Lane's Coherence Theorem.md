...
* **Morphisms.** For any two pure binary words $u,v$ of the same length, 
...
* **Morphisms.**
    * **(1)** If $\beta$ is an $\alpha$-arrow, we define 
    $\Phi_A(\beta) = (\beta)_A$.
    * **(2)** Now we define our functor on a general morphism $v \to w$ in $\wp$. For convenience 
...
* **Objects** We define the general proxy map on objects recursively.
    ...
    * For a binary word $w = u \otimes v$ we set:
    ...
    \]
* **Morphisms** We define the partial functor only on $\alpha$-, $\lambda$-, and $\rho$-arrows. 
...
* **On morphisms.** Let $F_1, G_1: \mm^n \to \mm$ and $F_2, G_2: \mm^m \to \mm$. 
