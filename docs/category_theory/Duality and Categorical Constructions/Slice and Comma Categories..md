...
* **Objects.** All pairs $(C, f: \textcolor{Purple}{A} \to C)$ 
...
* **Objects.** The objects are pairs $(X, x_0)$ with $X$ a topological space 
and $x_0 \in X$.
* **Morphisms.** A morphism $f: (X, x_0) \to (Y, y_0)$ is any continuous function 
...
* ****2.**** Consider again a group homomorphism $\phi: G \to H$, but this time 
...
to describe this is via slice categories.
    * ***i*.** Let $X$ be a set and $(Y, \tau)$ a topological space. 
    Denote $U: **Top** \to **Set**$ 
...
* ****4.****
    * ***i*.** Let $X$, $Y$ be topological spaces with $\phi: X \to Y$ a continuous function.
...
* ****5.**** In this exercise we'll see that slice categories describe intervals for thin categories.
