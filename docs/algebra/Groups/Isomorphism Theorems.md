...
* **Well-Defined.** Suppose $g' \in gK$. Then $gK = g'K$, and our goal will be to
...
* **It's a Homomorphism.** Let's justify that this is a homorphism. For $gK, g'K
\in G/K$, 
...
* **One-to-One.** To show this is one-to-one, we can use
...
one-to-one.
* **Onto.** To show this is onto, we'll simply show that
...
* **\phantom{1}** \hspace{0.5cm}  Construct a
...
* **\phantom{1}** \hspace{0.5cm} Consider a subgroup $M$
...
* **\phantom{1}** \hspace{0.5cm} Let $H, K$ be subgroups of $G$ such that $N
