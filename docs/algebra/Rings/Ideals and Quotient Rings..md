...
* **1.** By the Second Isomorphism Theorem we know that $I + J$
...
I'll follow that convention.)
    * **$\bm{\phi^{-1}(M)}$ is nonempty.** Observe that $\phi(0) = \dfrac{0}{1} + D^{-1}I \in M$,
    as $M$ is an ideal of $D^{-1}R/D^{-1}I$ and hence
    contains the zero element. Therefore $0 \in
    \phi^{-1}(M) = P$ and so
    $P$ is nonempty.
    * **$\bm{a, b \in \phi^{-1}(M)\implies a-b \in \phi^{-1}(M)}$.** Let $a, b \in \phi^{-1}(M)$. Then $\phi(a), \phi(b) \in M$.
    Hence, we see that 
...
* **Addition.** For any $r + I, s + I \in R/I$ we have
that 
