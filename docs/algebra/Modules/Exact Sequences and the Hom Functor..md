...
* **1.** ($\implies$) Suppose the sequence 
...
* **($\bm{1 \implies 3}$).** Suppose there exists an
...
of $M$.
* **($\bm{2 \implies 3}$).** Suppose (2) holds. We'll show that $\textcolor{blue}{m - \beta(g(m)) \in \ker(g)}$ for all $m \in M$. 
...
* **($\bm{1 \implies 2}$).** Suppose (1) holds. Construct a function $\beta: M_2 \to M$
defined by 
...
choice of $v$).
    * **Well-defined.** Suppose $g(v') = u$ for some other $v' \in M$. Then
    ...
    0$.
* **$\bm{(2 \implies 1)}$.** Suppose (2) holds. Construct
...
* **$\bm{f_*}$ is injective.** Suppose that $f_*(\psi) = 0$ for some $\phi \in
\hom_R(N, M_1)$. Then 
...
* **$\bm{\im(f_*) \subset \ker(g_*)}$.** Let $\phi \in \hom_R(N, M_1)$. Then observe that 
...
* **$\bm{\ker(g_*) \subset \im(f_*)}$.** Let $\psi \in \hom_R(N, M)$ and suppose that $g_*(\psi)
= 0$. Note that 
...
* **$\bm{f}$ is injective.** Suppose that sequence
...
* **$\im(f) \subset \ker(g)$.** Let $N = M_1$, and let $1_{M_1}:M_1 \to M_1$ be the
